...
$(document).ready(function () {
  ...
  $('[class^="swiper-category-container"]').each(function (index, container) {
    var $container = $(container); // Преобразуем в объект jQuery
    var $slides = $container.find('.swiper-slide'); // Находим слайды внутри контейнера

    if ($slides.length === 0) {
      console.warn("No slides found in container ".concat(index + 1));
    ...
    var swiper = new Swiper(container, {
      loop: $slides.length > 1,
      // Включить loop только если больше одного слайда
      spaceBetween: 10,
      slidesPerView: 'auto',
      // Подстраивается под ширину
      slidesPerGroup: 1,
      // По одному слайду за раз
      navigation: {
        nextEl: ".swiper-category-button-next".concat(index + 1),
        // Привязка к уникальным кнопкам
        prevEl: ".swiper-category-button-prev".concat(index + 1)
      },
      speed: 500,
      breakpoints: {
        600: {
    ...
    console.log("Swiper initialized for container ".concat(index + 1), swiper);
  });
});
/******/ })()
;